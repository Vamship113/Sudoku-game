...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupBoard() {
        ...
        switch (difficulty){
            case EASY:
                difficultyInt = 2;
                squaresLeftToComplete = 2;
                break;
            case MEDIUM:
                difficultyInt = 40;
                squaresLeftToComplete = 40;
                break;
            case HARD:
                difficultyInt = 55;
                squaresLeftToComplete = 55;
                break;
        }
                    ...
                    if (btnLockMap.get(currentSquare).equals("UnLocked")) {
                        ...
                        if (btnCheckMap.get(currentSquare) == Integer.parseInt(currentSquare.getText().toString())) {
                            ...
                            if (squaresLeftToComplete == 0){ //game complete
                                if (difficulty == Difficulty.HARD || difficulty == Difficulty.MEDIUM || difficulty == Difficulty.EASY ) {
                                    showPopup();
                                }
                            }
                        } else {
